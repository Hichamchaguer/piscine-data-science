...
def load():
    try:
        ...
        dayy, event_type, purchase_count = zip(*data)

        plt.figure(figsize=(12, 8))
        plt.plot(dayy, purchase_count, linestyle='-')
        plt.ylabel("Number of customers")
        plt.gca().yaxis.set_major_formatter(FuncFormatter(lambda x, pos: f'{int(x)}'))
        
        ax = plt.gca()
        ax.xaxis.set_major_locator(mdates.MonthLocator())
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%b'))  # Oct, Nov, Dec...

        plt.xlim(dayy[0], dayy[-1])
